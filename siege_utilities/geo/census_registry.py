...
scattered across ``config/census_constants.py``, ``census_api_client.py``,
and ``census_dataset_mapper.py``.

Moved from ``config/census_registry.py`` (SU#577) — these are domain
constants consumed by ``geo/``, not library configuration.  Backward-
compatible shims remain in ``config/census_constants.py`` and
``config/census_registry.py``.
"""
