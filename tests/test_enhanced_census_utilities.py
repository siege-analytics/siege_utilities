"""
Comprehensive unit tests for enhanced Census utilities.
...
class TestCensusDirectoryDiscovery:
    ...
    @patch('requests.get')
    def test_get_available_years_ssl_error_then_success(self, mock_get):
        """Test SSL error followed by successful fallback."""
        ...
        success_response.content = self.mock_html_content.encode()
        ...
        # Should get years from successful fallback
        assert 1990 in years
        assert 2020 in years
        assert len(years) > 0
    
    @patch('requests.get')
    def test_get_available_years_ssl_fallback(self, mock_get):
        """Test SSL fallback when discovery fails."""
        # First call fails with SSL error
        from requests.exceptions import SSLError
        ssl_error = SSLError("SSL error")
    ...
    def test_discover_boundary_types_with_variations(self):
        """Test boundary type discovery with naming variations."""
        # Mock the directory contents with variations
        self.discovery.get_year_directory_contents = Mock(return_value=[
            'STATE', 'COUNTY', 'TRACT', 'BG', 'TABBLOCK20', 'PLACE', 'ZCTA5',
            'CD118', 'SLDU', 'SLDL', 'CBSA', 'CSA', 'AIANNH'
        ])
        
        boundary_types = self.discovery.discover_boundary_types(2020)
        
        # Test specific congress number
        assert 'cd118' in boundary_types
        
        # Test legislative districts
        assert 'sldu' in boundary_types
        assert 'sldl' in boundary_types
        
        # Test statistical areas
        assert 'cbsa' in boundary_types
        assert 'csa' in boundary_types
        
        # Test tribal boundaries
        assert 'aiannh' in boundary_types
    ...
    def test_construct_download_url_place_with_state_fips(self):
        """Test URL construction for place boundaries with optional state FIPS."""
        # Mock available boundary types
        self.discovery.discover_boundary_types = Mock(return_value={
            'place': 'PLACE'
        })
        
        # Test without state FIPS (national)
        url = self.discovery.construct_download_url(2020, 'place')
        assert url == "https://www2.census.gov/geo/tiger/TIGER2020/PLACE/tl_2020_us_place.zip"
        
        # Test with state FIPS (state-specific)
        url = self.discovery.construct_download_url(2020, 'place', state_fips='06')
        assert url == "https://www2.census.gov/geo/tiger/TIGER2020/PLACE/tl_2020_06_place.zip"
...
class TestCensusDataSource:
    ...
    def test_get_state_abbreviations(self):
        ...
        abbreviations = self.census.get_state_abbreviations()
        
        assert abbreviations['06'] == 'CA'  # California
    ...
    def test_get_comprehensive_state_info(self):
        ...
        comprehensive = self.census.get_comprehensive_state_info()
        
        # Test California
        ca_info = comprehensive['06']
        assert ca_info['name'] == 'California'
        assert ca_info['abbreviation'] == 'CA'
        assert ca_info['fips'] == '06'
        
        # Test Texas
        tx_info = comprehensive['48']
    ...
    def test_get_state_by_abbreviation(self):
        ...
        ca_info = self.census.get_state_by_abbreviation('CA')
        assert ca_info['name'] == 'California'
        assert ca_info['abbreviation'] == 'CA'
        assert ca_info['fips'] == '06'
        
        tx_info = self.census.get_state_by_abbreviation('TX')
        assert tx_info['name'] == 'Texas'
        assert tx_info['abbreviation'] == 'TX'
        assert tx_info['fips'] == '48'
        
        # Test case insensitivity
        ca_info_lower = self.census.get_state_by_abbreviation('ca')
        assert ca_info_lower['name'] == 'California'
        
        # Test invalid abbreviations
        assert self.census.get_state_by_abbreviation('XX') is None
        assert self.census.get_state_by_abbreviation('') is None
        assert self.census.get_state_by_abbreviation('ABC') is None
    
    def test_get_state_by_name(self):
        """Test getting state information by name."""
        # Test exact matches
        ca_info = self.census.get_state_by_name('California')
        assert ca_info['name'] == 'California'
        assert ca_info['abbreviation'] == 'CA'
        assert ca_info['fips'] == '06'
        
        # Test partial matches
        ca_info_partial = self.census.get_state_by_name('Calif')
        assert ca_info_partial['name'] == 'California'
        
        tx_info_partial = self.census.get_state_by_name('tex')
        assert tx_info_partial['name'] == 'Texas'
        
        # Test case insensitivity
        ca_info_lower = self.census.get_state_by_name('california')
        assert ca_info_lower['name'] == 'California'
        
        # Test invalid names
        assert self.census.get_state_by_name('') is None
        assert self.census.get_state_by_name('InvalidState') is None
    
    def test_get_state_abbreviation(self):
        """Test getting state abbreviation from FIPS code."""
        assert self.census.get_state_abbreviation('06') == 'CA'  # California
        assert self.census.get_state_abbreviation('48') == 'TX'  # Texas
        assert self.census.get_state_abbreviation('36') == 'NY'  # New York
        assert self.census.get_state_abbreviation('11') == 'DC'  # District of Columbia
        assert self.census.get_state_abbreviation('99') is None  # Invalid FIPS
        assert self.census.get_state_abbreviation('') is None    # Empty string
    ...
    def test_validate_state_fips(self):
        ...
        assert self.census.validate_state_fips('') is False    # Empty string
    ...
    def test_get_state_name(self):
        ...
        assert self.census.get_state_name('') is None    # Empty string
    ...
    def test_parameter_validation_invalid_year(self):
        ...
        with pytest.raises(ValueError, match="Year 2030 is outside valid range"):
            self.census._validate_census_parameters(2030, 'county', None)
    ...
    def test_parameter_validation_missing_state_fips(self):
        ...
        with pytest.raises(ValueError, match="State FIPS required for block_group-level data"):
            self.census._validate_census_parameters(2020, 'block_group', None)
    ...
    def test_parameter_validation_invalid_state_fips(self):
        ...
        with pytest.raises(ValueError, match="Invalid state FIPS code: ABC"):
            self.census._validate_census_parameters(2020, 'tract', 'ABC')
    ...
    def test_state_required_levels(self):
        """Test that state-required levels are correctly identified."""
        state_required = self.census.state_required_levels
        
        assert 'tract' in state_required
        assert 'block_group' in state_required
        assert 'block' in state_required
        assert 'tabblock20' in state_required
        assert 'sldu' in state_required
        assert 'sldl' in state_required
    
    def test_national_levels(self):
        """Test that national levels are correctly identified."""
        national = self.census.national_levels
        
        assert 'nation' in national
        assert 'state' in national
        assert 'county' in national
        assert 'place' in national
        assert 'zcta' in national
        assert 'cd' in national
...
class TestErrorHandling:
    ...
    def test_discovery_with_empty_response(self):
        """Test discovery with empty HTML response."""
        ...
        with patch('requests.get') as mock_get:
            ...
            # When HTML parsing finds no years, it should return empty list
            # The fallback only happens on network errors, not parsing failures
            assert len(years) == 0
    
    def test_discovery_with_malformed_html(self):
        """Test discovery with malformed HTML."""
        discovery = CensusDirectoryDiscovery()
        
        with patch('requests.get') as mock_get:
            mock_response = Mock()
            mock_response.content = "This is not HTML at all".encode()
            mock_response.raise_for_status.return_value = None
            mock_get.return_value = mock_response
            
            years = discovery.get_available_years()
            # When HTML parsing fails, it should return empty list
            # The fallback only happens on network errors, not parsing failures
            assert len(years) == 0
    ...
    def test_parameter_validation_edge_cases(self):
        ...
        with pytest.raises(TypeError):
            census._validate_census_parameters(None, 'county', None)
        
        # Test with None geographic level - this should fail at the boundary type discovery
        # Mock the discovery to return empty boundary types
        census.discovery.discover_boundary_types = Mock(return_value={})
        
        with pytest.raises(ValueError):
            census._validate_census_parameters(2020, None, None)
        
        # Test with empty strings
        with pytest.raises(ValueError):
            census._validate_census_parameters(2020, '', None)
        
        with pytest.raises(ValueError):
            census._validate_census_parameters(2020, 'county', '')
